...
- [~] **Phase 4 — 절차적 씬 렌더링** *(진행 중)*: `Rendering/ScenePainter.cs`(Painter2D
      ...
      수면 5겹 파동·윤슬 + **찌·파문·안개 + 보트·어부·낚싯대·낚싯줄(베지어) + 바람·비
      streak + 대기 오버레이(보케 모트·블룸·안개띠) + 점프 물고기(SPLASHING 도약,
      facet 바디·꼬리 wag) + 물보라 입자(삼각/다이아/육각 shard, 중력·페이드)**.
      **씬 렌더 코드 전부 완료 — 육안 검증만 남음**(PanelSettings + Play). ⚠️ additive/
      라디얼 미지원 → 대기는 normal-alpha 근사, 비네팅 생략.
  - [~] **구도·아트 재설계(사수 결정, 진행 중)**: 측면 풍경 → **어깨너머 ~25° 구도**
        (하늘·먼 산·물·뒤에서 본 배+어부·한쪽 기슭·수면 아래 물고기·윤슬). 시간대/날씨는
        하늘·물·빛 톤으로 반영. 줌(스크롤/핀치) 완료.
      - [x] **배경 그라데이션 일러스트 격상**: Painter2D 미세 띠(3-stop 하늘·far→near 물)
            + 레이어드 해/달 글로우·수면 반사·대기 원근 산·비네팅. 시간대별 파라미터화로
            매끄러운 전환 유지. (목표 = feature-graphic 톤, ~75% 근접.)
      - [ ] **보트+어부 일러스트 스프라이트화 (다음 작업)**: 승인된 평면 일러스트
            `docs/art/boat_fisherman.svg`(뒷모습, 낚싯대 tip=256,114) → Vector Graphics
            패키지(`com.unity.vectorgraphics`)로 벡터 스프라이트 import → 씬에 오버레이
            (배치·줌 연동·bobbing, ~45° 기울임은 배치 시 적용). 낚싯줄/찌/파문은 Painter2D 유지.
            지금 ScenePainter의 `DrawBoatBehind`(거친 절차적 보트)를 이 스프라이트로 교체.
