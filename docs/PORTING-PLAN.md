...
- [~] **Phase 4 — 절차적 씬 렌더링** *(진행 중)*: `Rendering/ScenePainter.cs`(Painter2D
  ...
  - [~] **구도 재설계(사수 결정)**: 측면 풍경(원본 안드로이드 화면) → **탑다운 아늑한 연못**
        (앱 아이콘 무드 — 물이 화면을 채우고 위에서 본 배+어부 뒷모습, 한쪽 기슭 풀·모래·
        갈대·연잎, 수면 아래 헤엄치는 물고기, 윤슬). 시간대/날씨는 물·빛 톤으로 반영.
        `ScenePainter` 본문 재작성(`DrawScene` 시그니처 유지 → 렌더러/부트스트랩 무변경). **육안 조율 중.**
